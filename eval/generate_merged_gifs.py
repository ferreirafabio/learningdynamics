...
def create_merged_gifs_from_numpy(source_path):
    ...
    loaded_batch = load_all_experiments_from_dir(file_paths, use_with=False)
    ...
    for exp in loaded_batch.values():
        ...
        for step in exp.values():
            dnn_seg_output = step['seg']
            dnn_seg_output[dnn_seg_output >= 0.5] = 1.0
            dnn_seg_output[dnn_seg_output < 0.5] = 0.0
            dnn_seg_outputs.append(dnn_seg_output)

        gt_seg_data = [step['seg_target'].astype(np.float) for step in exp.values()]

        target_summaries_dict_seg = {
            prefix + '_target_seg_exp_id_{}'.format(exp_id): np.expand_dims(np.asarray(gt_seg_data), axis=4)}

        predicted_summaries_dict_seg = {
            prefix + '_predicted_seg_exp_id_{}'.format(exp_id): np.expand_dims(np.asarray(dnn_seg_outputs), axis=4)}
