...
csv_name = dataset_name.split("dataset_")[1] + ".csv"
...
with open(os.path.join(dir_path, csv_name), 'w') as csv_file:
    ...
    writer.writerow(["method name", "latent space data used", "estimator", "accuracy over 25% test set (n_samples)", "precision (global metric / micro)", "recall (global metric / micro)", "best params after 5CV grid search"])
