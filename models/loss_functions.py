...
def create_loss_ops(config, target_op, output_ops):
    ...
    if config.loss_type == 'cross_entropy_seg_only_edges':
        for i, output_op in enumerate(output_ops):
            ...
            target_edges_reshaped = _transform_edge_into_images(config, target_op.edges, img_type="seg")  # returns shape (?, 120,160,1)
        ...
        for i, output_op in enumerate(output_ops):
            ...
            loss_visual_ce_nodes = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
                                                        labels=labels,
                                                        logits=logits))
