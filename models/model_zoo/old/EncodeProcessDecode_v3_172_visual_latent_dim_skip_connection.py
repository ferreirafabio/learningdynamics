...
class EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection(snt.AbstractModule, BaseModel):
    """
  ...
  Input --->| Encoder |  *->| Core |--*->| Decoder |---> Output(t)
    ...
    def __init__(self, config, name="EncodeProcessDecode"):
        ...
        super(EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection, self).__init__(name=name)

        EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.convnet_pooling = config.convnet_pooling
        EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.convnet_tanh = config.convnet_tanh
        EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.depth_data_provided = config.depth_data_provided
        EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_conv_filters = config.n_conv_filters
        EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.model_id = config.model_type
        EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.latent_state_noise = config.latent_state_noise

        EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.edge_output_size = config.edge_output_size
        EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.node_output_size = config.node_output_size
        EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.global_output_size = config.global_output_size

        EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_layers_globals = config.n_layers_globals
        EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_layers_nodes = config.n_layers_nodes
        EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_layers_edges = config.n_layers_edges
        EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_neurons_edges = config.n_neurons_edges
        EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_neurons_globals = config.n_neurons_globals
        EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_neurons_nodes_non_visual = config.n_neurons_nodes_non_visual
        EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_neurons_nodes_total_dim = config.n_neurons_nodes_total_dim
    ...
    def _build(self, input_op, input_ctrl_ph, num_processing_steps, is_training):
        ...
        for step in range(0, num_processing_steps-1):
            ...
            decoded_op = self._decoder(latent, is_training, skip1=skip1, skip2=skip2)
...
class EncoderGlobalsGraphIndependent(snt.AbstractModule):
    ...
    def _build(self, inputs, is_training, verbose=VERBOSITY):
        ...
        self._network = modules.GraphIndependent(
            edge_model_fn=None,
            node_model_fn=None,
            global_model_fn=lambda: get_model_from_config(self.model_id, model_type="mlp")(n_neurons=EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_neurons_globals,
...
class CNNMLPEncoderGraphIndependent(snt.AbstractModule):
    ...
    def _build(self, inputs, is_training, verbose=VERBOSITY):
        ...
        self.visual_encoder = get_model_from_config(model_id=self.model_id, model_type="visual_encoder")(is_training=is_training, name="visual_encoder")
        self._network = modules.GraphIndependent(
            edge_model_fn=lambda: get_model_from_config(self.model_id, model_type="mlp")(n_neurons=EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_neurons_edges,
                                                                                         n_layers=EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_layers_edges,
                                                                                         output_size=None,
                                                                                         typ="mlp_layer_norm",
                                                                                         name="mlp_encoder_edge"),

            node_model_fn=lambda: get_model_from_config(self.model_id, model_type="visual_and_latent_encoder")(self.visual_encoder,
    ...
    def __init__(self, model_id, name="MLPGraphNetwork"):
        ...
        with self._enter_variable_scope():
          self._network = modules.GraphNetwork(
              edge_model_fn=lambda: get_model_from_config(model_id, model_type="mlp")(n_neurons=EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_neurons_edges,
                                                                                      n_layers=EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_layers_edges,
                                                                                      output_size=None,
                                                                                      typ="mlp_layer_norm",
                                                                                      name="mlp_core_edge"),
              node_model_fn=lambda: get_model_from_config(model_id, model_type="mlp")(n_neurons=EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_neurons_nodes_total_dim,
                                                                                      n_layers=EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_layers_nodes,
                                                                                      output_size=None,
                                                                                      typ="mlp_layer_norm",
                                                                                      name="mlp_core_node"),
              global_model_fn=lambda: get_model_from_config(model_id, model_type="mlp")(n_neurons=EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_neurons_globals,
                                                                                        n_layers=EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_layers_globals,
                                                                                        output_size=None,
                                                                                        typ="mlp_layer_norm",
...
class Decoder5LayerConvNet2D(snt.AbstractModule):
    ...
    def _build(self, inputs, name, verbose=VERBOSITY, keep_dropout_prop=0.6):
        filter_sizes = [EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_conv_filters, EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_conv_filters * 2]

        if EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.convnet_tanh:
            activation = tf.nn.tanh
        else:
            activation = tf.nn.relu

        #print(self.skip1.get_shape())
        #print(self.skip2.get_shape())

        img_shape = get_correct_image_shape(config=None, get_type='all', depth_data_provided=EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.depth_data_provided)

        """ get image data, get everything >except< last n elements which are non-visual (position and velocity) """
        image_data = inputs[:, :-EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_neurons_nodes_non_visual]
        ...
        ''' layer 1 (7,10,2) -> (7,10,filter_sizes[1])'''
        ...
        # --------------- SKIP CONNECTION ---------------
        outputs = tf.cond(tf.shape(outputs)[0] == tf.shape(self.skip2)[0],
                lambda: tf.concat([outputs, self.skip2], axis=1),
                lambda: outputs
                )
        ...
        ''' layer 8 (60,80,filter_sizes[0]) -> (60,80,filter_sizes[0]) '''
        outputs = tf.layers.conv2d_transpose(outputs, filters=filter_sizes[0], kernel_size=3, strides=1, padding='same')
        outputs = activation(outputs)
        outputs = tf.contrib.layers.layer_norm(outputs)
        ...
        ''' layer 9 (60,80,filter_sizes[0]) -> (120,160,filter_sizes[0]) '''
        outputs = tf.layers.conv2d_transpose(outputs, filters=filter_sizes[0], kernel_size=3, strides=2, padding='same')
        outputs = activation(outputs)
        outputs = tf.contrib.layers.layer_norm(outputs)
        l9_shape = outputs.get_shape()

        ''' layer 10 (120,160,filter_sizes[0]) -> (120,160,filter_sizes[0]) '''
        outputs = tf.layers.conv2d_transpose(outputs, filters=filter_sizes[0], kernel_size=3, strides=1, padding='same')
        outputs = activation(outputs)
        outputs = tf.contrib.layers.layer_norm(outputs)
        ...
        ''' layer 11 (120,160,filter_sizes[0]) -> (120,160,3 or 4 or 7]) '''
...
class Encoder5LayerConvNet2D(snt.AbstractModule):
    ...
    def _build(self, inputs, name, verbose=VERBOSITY, keep_dropout_prop=0.6):
        ...
        ''' layer 3'''
        if EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.convnet_pooling:
        ...
        ''' layer 11'''
        visual_latent_output = tf.layers.dense(inputs=visual_latent_output, units=EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_neurons_nodes_total_dim - EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_neurons_nodes_non_visual)
...
class VisualAndLatentDecoder(snt.AbstractModule):
    ...
    def _build(self, inputs, is_training=True, verbose=VERBOSITY):
        ...
        non_visual_decoded_output = snt.Sequential([snt.nets.MLP([EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_neurons_nodes_total_dim, n_non_visual_elements],
...
class VisualAndLatentEncoder(snt.AbstractModule):
    ...
    def _build(self, inputs, verbose=VERBOSITY):
        ...
        non_visual_latent_output = snt.Sequential([snt.nets.MLP([n_non_visual_elements, EncodeProcessDecode_v3_172_visual_latent_dim_skip_connection.n_neurons_nodes_non_visual],
        ...
        # todo: add noise to latent vector
