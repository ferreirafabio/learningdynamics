...
class baseline_auto_predictor_extended_multistep_position(BaseModel):
    def __init__(self, config, name="baseline_auto_predictor_extended_multistep_position"):
        super(baseline_auto_predictor_extended_multistep_position, self).__init__(self)
        self.config = config
    ...
    def cnnmodel(self, in_rgb, in_segxyz, in_control=None, is_training=True, n_predictions=5):
        ...
        for i in range(n_predictions):
            ...
            latent_img, obj_pos = self.physics_predictor(latent=latent_img, ctrl=in_control_T[i])
            img_decoded = self.decoder(latent=latent_img, is_training=is_training)

            predictions.append(img_decoded)
            out_pos.append(obj_pos)
