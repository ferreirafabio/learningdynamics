...
class Model_naive_xyz_net(BaseModel):
    def __init__(self, config, name="Model_naive_xyz_net"):
        super(Model_naive_xyz_net, self).__init__(self)
        self.config = config
    ...
    def cnnmodel(self, in_rgb, in_segxyz, in_control=None, is_training=True):
      ...
      x = tflearn.layers.conv.conv_2d(x,1024,(3,3),strides=2,activation='relu',weight_decay=1e-5,regularizer='L2')
      print("x before", x)
      ...
      x = tflearn.layers.normalization.batch_normalization(x)
      x = tflearn.layers.core.fully_connected(x,1024,activation='relu',bias=False)
      ...
      x = tflearn.layers.conv.conv_2d_transpose(x,1024,[3,3],[1,2],strides=2,activation='relu',weight_decay=1e-5,regularizer='L2')
      x = tflearn.layers.conv.conv_2d_transpose(x,1024,[3,3],[2,3],strides=2,activation='relu',weight_decay=1e-5,regularizer='L2')

      x = tflearn.layers.normalization.batch_normalization(x)
      x = tflearn.layers.conv.conv_2d_transpose(x,512,[3,3],[4,5],strides=2,activation='relu',weight_decay=1e-5,regularizer='L2')
      x = tflearn.layers.conv.conv_2d_transpose(x,512,[3,3],[8,10],strides=2,activation='relu',weight_decay=1e-5,regularizer='L2')
      x = tflearn.layers.conv.conv_2d_transpose(x,512,[3,3],[15,20],strides=2,activation='relu',weight_decay=1e-5,regularizer='L2')

      x = tflearn.layers.normalization.batch_normalization(x)
      x = tflearn.layers.conv.conv_2d_transpose(x,128,[5,5],[30,40],strides=2,activation='relu',weight_decay=1e-5,regularizer='L2')
      x = tflearn.layers.normalization.batch_normalization(x)
