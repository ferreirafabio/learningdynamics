...
class baseline_auto_predictor_extended_multistep_no_prev_step(BaseModel):
    def __init__(self, config, name="baseline_auto_predictor_extended_multistep_no_prev_step"):
        super(baseline_auto_predictor_extended_multistep_no_prev_step, self).__init__(self)
        self.config = config
    ...
    def physics_predictor(self, latent, ctrl):
        with tf.variable_scope("auto_predictor_multistep_extended", reuse=tf.AUTO_REUSE):
            ...
            if self.config.use_f_interact:
                ...
                physics_output = latent_next_step + f_interact_total
            else:
