...
class baseline_auto_predictor_extended_multistep_bigger(BaseModel):
    def __init__(self, config, name="baseline_auto_predictor_extended_multistep_bigger"):
        super(baseline_auto_predictor_extended_multistep_bigger, self).__init__(self)
        self.config = config
    ...
    def encoder(self, in_rgbsegxyz, is_training):
        x = in_rgbsegxyz
        """ Layer 1 """
        x = tflearn.layers.conv.conv_2d(x, 256, (3, 3), strides=1, activation='relu', weight_decay=1e-5, regularizer='L2', scope="conv1_1")
        x = tflearn.layers.normalization.batch_normalization(x)

        """ Layer 2 """
        x = tflearn.layers.conv.conv_2d(x, 256, (3, 3), strides=1, activation='relu', weight_decay=1e-5, regularizer='L2', scope="conv1_2")
        x = tflearn.layers.normalization.batch_normalization(x)
        ...
        """ Layer 8 """
        x = tflearn.layers.conv.conv_2d(x, 512, (3, 3), strides=1, activation='relu', weight_decay=1e-5, regularizer='L2', scope="conv1_8")
        x = tflearn.layers.normalization.batch_normalization(x)
        x = tflearn.layers.conv.max_pool_2d(x, 2, 2)

        """ Layer 9 """
        x = tflearn.layers.conv.conv_2d(x, 512, (3, 3), strides=2, activation='relu', weight_decay=1e-5, regularizer='L2', scope="conv1_9")
        x = tflearn.layers.normalization.batch_normalization(x)
    ...
    def decoder(self, latent, is_training):
        ...
        with tf.variable_scope("auto_predictor_multistep_extended", reuse=tf.AUTO_REUSE):
            ...
            """ Layer 1 """
            x = tflearn.layers.conv.conv_2d_transpose(x, output_shape=[2, 2], nb_filter=512, filter_size=2, strides=1, activation='relu', weight_decay=1e-5, regularizer='L2', padding='valid', name='decoder_transpose_1')
            x = tflearn.layers.normalization.batch_normalization(x, name='decoder_bn_1')

            """ Layer 2 """
            x = tflearn.layers.conv.conv_2d_transpose(x, output_shape=[4, 4], nb_filter=512, filter_size=2, strides=2, activation='relu', weight_decay=1e-5, regularizer='L2', name='decoder_transpose_2')
            x = tflearn.layers.normalization.batch_normalization(x, name='decoder_bn_2')

            """ Layer 3 """
            x = tflearn.layers.conv.conv_2d_transpose(x, nb_filter=512, output_shape=[7, 10], filter_size=4, strides=[1, 2], activation='relu', weight_decay=1e-5, regularizer='L2', padding="valid", name='decoder_transpose_3')
            ...
            """ Layer 6 """
            x = tflearn.layers.conv.conv_2d_transpose(x, output_shape=[15, 20], nb_filter=256, filter_size=2, strides=1, activation='relu', weight_decay=1e-5, regularizer='L2', name='decoder_transpose_6')
            x = tflearn.layers.normalization.batch_normalization(x, name='decoder_bn_6')

            """ Layer 7 """
            x = tflearn.layers.conv.conv_2d_transpose(x, output_shape=[30, 40], nb_filter=256, filter_size=2, strides=2, activation='relu', weight_decay=1e-5, regularizer='L2', name='decoder_transpose_7')
    ...
    def physics_predictor(self, latent, ctrl):
        with tf.variable_scope("auto_predictor_multistep_extended", reuse=tf.AUTO_REUSE):
            ...
            """ control MLP """
            latent_ctrl = tflearn.layers.core.fully_connected(ctrl, 64, activation='relu', name='ctrl_mlp_fc_1')
            latent_ctrl = tflearn.layers.normalization.batch_normalization(latent_ctrl, name='ctrl_mlp_bn_1')

            latent_ctrl = tflearn.layers.core.fully_connected(latent_ctrl, 64, activation='relu', name='ctrl_mlp_fc_2')
            latent_ctrl = tflearn.layers.normalization.batch_normalization(latent_ctrl, name='ctrl_mlp_bn_2')

            latent_ctrl = tflearn.layers.core.fully_connected(latent_ctrl, 64, activation='relu', name="ctrl_mlp_fc_3")
