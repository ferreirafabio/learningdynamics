...
class baseline_auto_predictor_extended_multistep_lstm(BaseModel):
    def __init__(self, config, name="baseline_auto_predictor_extended_multistep_lstm"):
        super(baseline_auto_predictor_extended_multistep_lstm, self).__init__(self)
        self.config = config
    ...
    def cnnmodel(self, in_rgb, in_segxyz, in_control=None, is_training=True, n_predictions=5):
        ...
        """ control MLP """
        latent_ctrl = tflearn.layers.core.fully_connected(in_control, 32, activation='relu', name='ctrl_mlp_fc_1')
        ...
        zero_padding = tf.zeros([tf.shape(latent_ctrl)[0], 1, 256])
