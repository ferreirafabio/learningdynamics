...
def cnnmodel(inp_rgb, inp_stepsize=None, is_training=True):
    ...
    feat = feat_C4
    feat = tflearn.layers.conv.conv_2d(feat, 1024, (1, 1), strides=1, activation='relu', weight_decay=1e-5,
                                       regularizer='L2')

    feat_0 = tflearn.layers.conv.avg_pool_2d(feat, [8, 8], [8, 8], padding='VALID')
    feat_0 = tflearn.layers.conv.conv_2d(feat_0, 256, (1, 1), strides=1, activation='relu', weight_decay=1e-5,
                                         regularizer='L2')
    feat_0 = tf.image.resize_bilinear(feat_0, [8, 8], align_corners=True)

    feat_1 = tflearn.layers.conv.conv_2d(feat, 256, (1, 1), strides=1, activation='relu', weight_decay=1e-5,
                                         regularizer='L2')

    feat_2 = tflearn.layers.conv.atrous_conv_2d(feat, 256, (3, 3), rate=2, activation='relu', weight_decay=1e-5,
                                                regularizer='L2')

    feat_4 = tflearn.layers.conv.atrous_conv_2d(feat, 256, (3, 3), rate=4, activation='relu', weight_decay=1e-5,
                                                regularizer='L2')

    feat_6 = tflearn.layers.conv.atrous_conv_2d(feat, 256, (3, 3), rate=6, activation='relu', weight_decay=1e-5,
                                                regularizer='L2')

    feat_ac = tf.concat([feat_0, feat_1, feat_4, feat_6], axis=-1)
    feat_ac = tflearn.layers.conv.conv_2d(feat_ac, 256, (1, 1), strides=1, activation='relu', weight_decay=1e-5,
                                          regularizer='L2')
